...
class HomeModelImp : HomeModel {
    ...
    override fun getEventData(callback: DataFatchCallback<EventData>) {
        val userpro = FirebaseAuth.getInstance().currentUser!!.uid
        val db1 =FirebaseDatabase.getInstance().getReference("User").child(userpro).child("events")

        db1.addValueEventListener(object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                val data = snapshot.getValue(EventData::class.java)
                callback.onSuccess(data!!)
            }
